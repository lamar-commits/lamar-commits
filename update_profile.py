...
USER = "lamar-commits"
ACCOUNT_CREATED = date(2026, 4, 22)  # GitHub account creation, drives Uptime
JOINED_YEAR = 2026  # first year to tally contributions from
...
def tenure(created, today):
    y, m, d = span(created, today)
    parts = []
    if y:
...
def info_lines(s):
    ...
    return [
        ...
        [],
        kv("OS", "Windows, macOS"),
        kv("Uptime", tenure(ACCOUNT_CREATED, date.today())),
        kv("Host", "Olympus"),
        kv("Kernel", "Founder-Operator"),
        kv("IDE", "Claude Code, Cursor, VS Code"),
        [],
        kv("Languages.Programming", "Python, TypeScript, SQL, PowerShell"),
        kv("Languages.Real", "English"),
        kv("Focus", "AI agents, systems, automation"),
        [],
        rule("GitHub Stats"),
...
def render(mode, stats):
    ...
    art_bottom = 40 + (len(art) - 1) * 15 + 8
    info_bottom = 45 + (len(info) - 1) * 21 + 8
    h = max(art_bottom, info_bottom) + 14
    out = [
        f'<svg xmlns="http://www.w3.org/2000/svg" width="840" height="{h}" viewBox="0 0 840 {h}" '
        f'font-family="Consolas, Menlo, monospace" font-size="13px">',
        f'<rect x="0.5" y="0.5" width="839" height="{h - 1}" rx="10" fill="{p["bg"]}" stroke="{p["border"]}"/>',
    ]
...
def selfcheck():
    assert span(date(1989, 1, 15), date(2026, 7, 10)) == (37, 5, 25)
    assert span(date(2000, 3, 31), date(2026, 4, 1)) == (26, 0, 1)
    assert span(date(2000, 1, 1), date(2026, 1, 1)) == (26, 0, 0)
